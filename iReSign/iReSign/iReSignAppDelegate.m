    ...
    if (appPath) {
        ...
        for (NSString *file in workingDirContents) {
            ...
            // 排除entitlements.plist和其他签名临时文件
                     ...
                     ![file containsString:@"Entitlements"]) {
                // 只添加非签名相关的plist文件
                NSLog(@"Adding metadata file: %@", file);
                [filesToZip addObject:file];
            }
        }
        ...
        // 使用文件列表而不是打包整个目录
        NSMutableArray *arguments = [NSMutableArray arrayWithObjects:@"-qry", destinationPath, nil];
        [arguments addObjectsFromArray:filesToZip];
        ...
        NSLog(@"Zipping files: %@ to %@", [filesToZip componentsJoinedByString:@", "], destinationPath);
